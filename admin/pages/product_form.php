...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($blob)
                $ins->execute([$product_id, $blob['data'], $blob['mime'], $next_sort++]);
        }
...
<style>
    ...
    .gallery-new-row .img-tabs {
        border-bottom: 1px solid #bbf7d0;
    }

    .gallery-new-row .img-tab {
        font-size: 12px;
        padding: 8px;
    }

    .gallery-new-row .img-panel {
        padding: 12px;
    }

    .gallery-new-row .file-drop-zone {
        padding: 18px 12px;
        font-size: 12px;
    }

    .gallery-new-row .file-drop-zone i {
        font-size: 20px;
    }
...
<form method="POST" action="index.php?page=product-form" enctype="multipart/form-data" class="product-form-wrap">
    ...
    <div class="form-section">
        ...
        <div class="form-grid-2">
            ...
            <div class="form-group form-group-full">
                ...
                <input type="text" name="name" id="f_name" value="<?= htmlspecialchars($product['name'] ?? '') ?>"
                    placeholder="VD: Áo thun basic unisex" maxlength="255" required>
            </div>

            <div class="form-group form-group-full">
                <label>Slug <span class="req">*</span></label>
                <input type="text" name="slug" id="f_slug" value="<?= htmlspecialchars($product['slug'] ?? '') ?>"
            ...
            <div class="form-group">
                ...
                <input type="number" name="stock" value="<?= $product['stock'] ?? '' ?>" min="0" placeholder="0"
                    required>
            </div>
    ...
    <div class="form-section">
        ...
        <div class="img-widget">
            <div class="img-preview-bar <?= $has_image ? 'has-image' : '' ?>" id="mainPreviewBar">
                <img id="mainPreviewImg" src="<?= $has_image ? htmlspecialchars(img_url($product_id)) : '' ?>" alt="">
            ...
            <div class="img-panel <?= $has_image ? 'hidden' : '' ?>" id="main-file-panel">
                <label class="file-drop-zone" for="mainFileInput">
                    ...
                    <input type="file" name="image_file" id="mainFileInput" accept="image/*"
                        onchange="onFileChange(this,'main')">
                </label>
            </div>
            <div class="img-panel <?= !$has_image ? 'hidden' : '' ?>" id="main-url-panel">
                <input type="text" name="image_url" class="url-input" placeholder="https://example.com/image.jpg"
    ...
    <div class="form-section">
        ...
        <div class="img-widget">
            <div class="img-preview-bar <?= $has_thumb ? 'has-image' : '' ?>" id="thumbPreviewBar">
                <img id="thumbPreviewImg" src="<?= $has_thumb ? htmlspecialchars(img_url($product_id, 'thumb')) : '' ?>"
                    alt="">
            ...
            <div class="img-panel <?= $has_thumb ? 'hidden' : '' ?>" id="thumb-file-panel">
                <label class="file-drop-zone" for="thumbFileInput">
                    ...
                    <span>Kéo thả hoặc bấm để chọn ảnh</span>
                    <small>JPG, PNG, WebP, GIF — tối đa 10MB</small>
                    <input type="file" name="thumbnail_file" id="thumbFileInput" accept="image/*"
                        onchange="onFileChange(this,'thumb')">
                </label>
            </div>
            <div class="img-panel <?= !$has_thumb ? 'hidden' : '' ?>" id="thumb-url-panel">
                <input type="text" name="thumbnail_url" class="url-input" placeholder="https://example.com/thumb.jpg"
        ...
        <div class="gallery-existing" id="galleryExisting">
            <?php foreach ($gallery as $gi): ?>
                <div class="gallery-row" id="gi_<?= $gi['id'] ?>">
                    <input type="hidden" name="delete_gallery_ids[]" value="" disabled data-gid="<?= $gi['id'] ?>">
                    <img src="<?= htmlspecialchars(gallery_url((int) $gi['id'])) ?>" class="gallery-thumb"
...
<script>
    (function () {
        ...
        const widgetDef = {
            main: {
                previewBar: 'mainPreviewBar', previewImg: 'mainPreviewImg',
                filePanel: 'main-file-panel', urlPanel: 'main-url-panel',
                fileInput: 'mainFileInput', clearHidden: 'clearImage'
            },
            thumb: {
                previewBar: 'thumbPreviewBar', previewImg: 'thumbPreviewImg',
